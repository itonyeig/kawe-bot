...
export  async function getBookRecommendations(childAge: string | number, bookList: string[], orderHistory: string[], questionsAnswers: QuestionsAnswers[]): Promise<string> {
    
  
    try {
     // Prepare the prompt for OpenAI
     let prompt = `Based on the specific details provided, recommend books for a child, ensuring that each recommendation comes exclusively from the provided Book List, which primarily consists of African-themed books. Include a concise reason for each recommendation, focusing on the child's age, previous order history, and their questions and answers. Use the 👉 symbol for each recommendation.
     ...
     Important: It is crucial to recommend books only from the Book List, with a focus on African themes. Avoid suggesting any books not on the list. Provide a brief rationale for each recommended book.
